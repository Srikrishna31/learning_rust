...
fn main() {
    ...
    // so the following code creates a vector of zeros whose length is bounds.0*bounds.1, where
    //bounds is the image resolution parsed from the command line.
...
fn test_parse_pair() {
    ...
    assert_eq!(parse_pair("10.0x20.0", 'x'),Some((10.0, 20.0)));
...
/// The `upper_left` and lower_left` parameters are points on the complex plane designating the
...
-> Complex<f64>
{
    ...
    //<> as f64 is Rust syntax for type conversion; Rust generally refuses to convert between
    ...
    Complex{re: upper_left.re+pixel.0 as f64 * width / bounds.0 as f64,
            ...
            //Why subtraction here? pixel.1 increases as we go down, but the imaginary
...
{
    // To properly handle File::create's result, we need to match on its return value, as below:
    ...
    //  }
    //};
    //This kind of match statement is such a common pattern in Rust that the language provides the ?
    // operator as shorthand for the whole thing.
